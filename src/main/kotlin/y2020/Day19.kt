...
class Day19 : Day() {
    ...
    private fun String.verify(rules: Map<Int, Rule>, pda: List<Int>): Boolean {
        ...
        return when (val rule = rules[pda.first()]!!) {
            is Letter -> startsWith(rule.char) && drop(1).verify(rules, pda.drop(1))
            is Choice -> rule.choices.firstOrNull { choice -> verify(rules, choice + pda.drop(1)) } != null
        }
    }

    private fun parseRules(str: String): Pair<Int, Rule> {
        val (num, prod) = str.split(':').map(String::trim)
        if (prod.startsWith('"')) return num.toInt() to Letter(prod[1])
        return num.toInt() to Choice(
            prod.trim()
                .split('|')
                .map(String::trim)
