...
class Day06 : Day<List<List<Long>>>(2023, 6) {
    ...
    override suspend fun solve1(input: List<List<Long>>) = times.indices.map { times[it].countWins(highscores[it]) }.reduce { acc, i -> acc * i }

    override suspend fun solve2(input: List<List<Long>>): Int {
