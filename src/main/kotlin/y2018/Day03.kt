...
class Day03 : Day<Set<Day03.Claim>>(2018, 3) {
    ...
    override suspend fun List<String>.parse() = parseClaims()

    override suspend fun solve1(input: Set<Claim>) = input.flatMap { it.getPoints() }.groupBy { it }.values.count { it.size > 1 }

    override suspend fun solve2(input: Set<Claim>) = input.first { claim ->
